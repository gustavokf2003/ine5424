...
void Thread::dispatch(Thread * prev, Thread * next, bool charge)
{
    ...
    if(charge) {
        if(Criterion::timed){
            _timer->restart();
            if((next->criterion()._priority >= Scheduling_Criterion_Common::PERIODIC) && (next->criterion()._priority < Scheduling_Criterion_Common::APERIODIC)) {
                next->criterion().update();
            }
            if((prev->criterion()._priority >= Scheduling_Criterion_Common::PERIODIC) && (prev->criterion()._priority < Scheduling_Criterion_Common::APERIODIC)) {
